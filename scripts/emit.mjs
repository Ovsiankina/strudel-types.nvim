...
// Methods already on String.prototype / Number.prototype (incl. Object.prototype).
// Strudel mini-notation strings are patterns at runtime, so we mirror the pattern
// methods onto String/Number — but skip these to avoid TS "incompatible override".
const OBJECT_PROTO = 'constructor hasOwnProperty isPrototypeOf propertyIsEnumerable toLocaleString toString valueOf';
const STRING_PROTO = new Set(
  (OBJECT_PROTO + ' anchor at big blink bold charAt charCodeAt codePointAt concat ' +
    'endsWith fixed fontcolor fontsize includes indexOf italics lastIndexOf length ' +
    'link localeCompare match matchAll normalize padEnd padStart repeat replace ' +
    'replaceAll search slice small split startsWith strike sub substr substring sup ' +
    'toLocaleLowerCase toLocaleUpperCase toLowerCase toUpperCase trim trimEnd ' +
    'trimStart isWellFormed toWellFormed')
    .split(/\s+/),
);
const NUMBER_PROTO = new Set(
  (OBJECT_PROTO + ' toExponential toFixed toLocaleString toPrecision').split(/\s+/),
);
...
for (const nm of names) {
  ...
  const methodDecl = `${docM}\n    ${nm}(${args}): Pattern;`;
  methods.push(methodDecl);
  // Mirror onto String/Number so mini-notation literals chain: "bd*4".s().fast(2).
  // String keeps full docs (you hover the first method on a string); Number is bare.
  // Skip names that already exist on those prototypes (avoids override conflicts).
  if (!STRING_PROTO.has(nm)) stringMethods.push(methodDecl);
  if (!NUMBER_PROTO.has(nm)) numberMethods.push(`    ${nm}(${args}): Pattern;`);
}

// Hand-declared API that isn't in doc.json (see overrides.extras): undocumented
// globals (setcps, all, ...) and the underscore draw-only Pattern methods.
let nExtra = 0;
for (const nm of Object.keys(extras).sort()) {
  if (api.has(nm)) continue; // doc.json wins; never double-declare
  const x = extras[nm];
  const args = x.args != null ? x.args : '...args: any[]';
  const docG = jsdocComment(x.desc || '', x.examples, '  ');
  const docM = jsdocComment(x.desc || '', x.examples, '    ');
  if (x.global !== false && !RESERVED.has(nm) && !GLOBAL_BUILTINS.has(nm)) {
    globals.push(`${docG}\n  const ${nm}: (${args}) => Pattern;`);
  }
  if (x.method) {
    methods.push(`${docM}\n    ${nm}(${args}): Pattern;`);
    if (!STRING_PROTO.has(nm)) stringMethods.push(`${docM}\n    ${nm}(${args}): Pattern;`);
    if (!NUMBER_PROTO.has(nm)) numberMethods.push(`    ${nm}(${args}): Pattern;`);
  }
  nExtra++;
}
...
declare global {
  ...
  interface Pattern {
    /**
     * Fallback so unknown methods (e.g. your own \`register('name', ...)\` ones)
     * keep the chain typed as Pattern instead of collapsing to \`any\`.
     */
    [method: string]: (...args: any[]) => Pattern;
${methods.join('\n')}
  }

  // Strudel's transpiler turns mini-notation string (and number) literals into
  // patterns, so pattern methods are callable directly on them:
  //   "bd*4".s().fast(2)   "<0 1>".pickRestart([...])   (3).fast(2)
  // We mirror the pattern methods onto String/Number so those chains type-check.
  interface String {
${stringMethods.join('\n')}
  }
  interface Number {
...
console.error(`emit: ${names.length} names (${nFn} auto, ${nOvr} tuned, ${nSig} signals) + ${nExtra} extras`);
